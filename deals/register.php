...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // 必須項目チェック
  if (empty($type) || empty($issueDate) || empty($amount) || empty($accountItemId) || empty($taxCode)) {
    $error = '必須項目が入力されていません。';
  } else {
    setCompanyId($companyId);
    $options = [];
    if (!empty($dueDate)) $options['due_date'] = $dueDate;
    if (!empty($refNumber)) $options['ref_number'] = $refNumber;
    ...
    if (!empty($fromWalletableId)) {
      if ($fromWalletableId === 'private_account_item') {
        ...
        if ($privateAccountItemId) {
          ...
          if ($type === 'income') {
            $errors[] = 'プライベート資金用の勘定科目（事業主貸）が見つかりません。事業主貸の勘定科目を作成してください。';
          } else {
            $errors[] = 'プライベート資金用の勘定科目（事業主借）が見つかりません。事業主借の勘定科目を作成してください。';
          }
        }
      } else {
        ...
        if ($walletInfo) {
          ...
          $errors[] = '指定された決済口座が見つかりません。';
        }
      }
    }
    
    $detailOptions = [];
    if (!empty($description)) $detailOptions['description'] = $description;
    if (!empty($itemId)) $detailOptions['item_id'] = $itemId;
...
<div class="container">
  ...
  <div class="alert alert-info">
    <b>事業所：</b> <?php echo htmlspecialchars($selectedCompany['display_name']); ?>（ID: <?php echo htmlspecialchars($selectedCompany['id']); ?>）
  </div>
  <?php if ($error): ?>
    ...
    <form method="POST" class="deal-form">
      ...
      <div class="form-row">
        <div class="form-group col-md-6">
          ...
          <input type="date" name="issue_date" id="issue_date" class="form-control" value="<?php echo htmlspecialchars($_POST['issue_date'] ?? date('Y-m-d')); ?>" required>
        </div>
        <div class="form-group col-md-6">
          <label for="due_date">支払期日</label>
          <input type="date" name="due_date" id="due_date" class="form-control" value="<?php echo htmlspecialchars($_POST['due_date'] ?? ''); ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group col-md-6">
          <label for="amount">金額 <span class="required">*</span></label>
          <input type="number" name="amount" id="amount" class="form-control" value="<?php echo htmlspecialchars($_POST['amount'] ?? ''); ?>" min="1" step="1" required>
        </div>
        <div class="form-group col-md-6">
      ...
      <div class="form-group">
        ...
        <input type="text" name="ref_number" id="ref_number" class="form-control" value="<?php echo htmlspecialchars($_POST['ref_number'] ?? ''); ?>" maxlength="255">
        <small class="form-text text-muted">管理用の番号を入力してください（オプション）</small>
      </div>
      <div class="form-group">
        <label for="description">備考</label>
        <textarea name="description" id="description" class="form-control" rows="3" maxlength="500"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
      </div>
